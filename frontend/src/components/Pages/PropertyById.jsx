...
const PropertyById = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(0); // Track the current image index for auto-switching
  ...
  useEffect(() => {
    ...
    const fetchRatings = async () => {
      ...
      try {
        ...
        setAverageRating(avgRatingRes.data.averageRating || avgRatingRes.data);

        if (token) {
          const userRatingRes = await axios.get(
            `/api/v1/property/${id}/getUserRating`,
            {
              headers: {
          ...
          setUserRating(userRatingRes.data.rating || userRatingRes.data);
          setNewRating(userRatingRes.data.rating || userRatingRes.data);
        }
      } catch (error) {
  ...
  // Auto-switch the main image every 3 seconds
  useEffect(() => {
    if (property?.propertyImg?.length > 0) {
      const interval = setInterval(() => {
        setCurrentIndex((prevIndex) =>
          prevIndex === property.propertyImg.length - 1 ? 0 : prevIndex + 1
        );
      }, 3000); // Change the image every 3 seconds

      // Set the selected image to the current index
      setSelectedImage(property.propertyImg[currentIndex]?.data);
  ...
  const submitRating = async () => {
    ...
    if (newRating !== null) {
      try {
        ...
        setAverageRating(avgRating.data.averageRating || avgRating.data);
        setUserRating(userRating.data.rating || userRating.data);
        setNewRating(null);
      } catch (error) {
              ...
              {property?.propertyImg?.map((img, index) => (
                <img
                  ...
                  onClick={() => {
                    ...
                    setCurrentIndex(index); // Update index when thumbnail is clicked
                  }}
                  className="thumbnail"
                />
              ))}
